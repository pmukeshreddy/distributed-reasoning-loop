...
class GRPOConfig:
    ...
    save_lora_only: bool = False  # Save LoRA adapter without merging
    lora_checkpoint_dir: str = "./checkpoints/lora"
    continuous_mode: bool = False  # Enable for continuous training loop
...
class ReasoningGRPOTrainer:
    ...
    def save(self, path: Optional[str] = None, merge_lora: bool = True):
        ...
        if self.config.use_lora:
            if merge_lora and not self.config.save_lora_only:
                # Merge LoRA weights and save full model
                logger.info("Merging LoRA weights...")
                merged_model = self.model.merge_and_unload()
                merged_model.save_pretrained(save_path)
            else:
                # Save LoRA adapter only (for hot-reload)
                logger.info("Saving LoRA adapter only (no merge)...")
    ...
    def save_lora_only(self, path: Optional[str] = None) -> str:
        """
        Save only the LoRA adapter without merging.
        For use with SGLang hot-reload.
        
        Returns:
            Path where the LoRA adapter was saved
        """
        save_path = path or self.config.lora_checkpoint_dir
        Path(save_path).mkdir(parents=True, exist_ok=True)
        ...
        if self.config.use_lora:
            ...
            logger.warning("save_lora_only called but use_lora=False")
            self.model.save_pretrained(save_path)
        
        return save_path
    
    def train_step(self, data: List[Dict]) -> float:
        """
        Perform a single training step for continuous training.
        
        Args:
            data: List of samples with 'prompt', 'reasoning', 'is_correct' keys
        ...
        # Create temporary dataset from the data
        dataset = GRPODataset(
            data,
            self.tokenizer,
            max_length=self.config.max_length,
        ...
        if len(dataset) == 0:
            logger.warning("No valid training groups in data")
            return 0.0
    ...
    def load_lora_checkpoint(self, lora_path: str):
        """
        ...
        # If model already has LoRA, we need to merge and re-apply
        ...
        logger.info(f"LoRA checkpoint loaded from {lora_path}")
